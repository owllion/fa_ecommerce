...
def add_item_to_cart():
    cart = (
        session.query(cart_model.Cart)
        .filter(cart_model.Cart.user_id == "b57d552082804a55823b0d49ec1e2082")
        .first()
    )
    ...
    cart_item = (
        session.query(cart_item_model.CartItem)
        .filter(
            cart_item_model.CartItem.product_id == product_id,
            cart_item_model.CartItem.cart_id == cart_id,
        )
        .first()
    )
    ...
    if cart_item:
        ...
        # 找產品
        product = (
            session.query(product_model.Product)
            .filter(product_model.Product.id == "3b4f0e2dd1384cb5be045d66c31ab08f")
            .first()
        )
