...
def find_order_item(order_id: str, product_id: str, size: str, db: Session):
    order_item = (
        db.query(order_item_model.OrderItem)
        .filter(
...
def order_item_exists(order_id: str, product_id: str, size: str, db: Session):
...
def get_order_item_or_raise_not_found(order_id: str, product_id: str, size: str, db: Session):
    order_item = find_order_item(order_id, product_id, size, db)
    if order_item:
        return order_item
...
def delete_order_item_record(order_item: order_item_model.OrderItem, db: Session):
