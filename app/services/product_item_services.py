...
def find_product_item(product_id: str, size_id: str, db: Session):
    product_item = (
...
def product_item_not_exists(product_id: str, size_id: str, db: Session):
    product_item = find_product_item(product_id, size_id, db)
    if not product_item:
        return True

    raise_http_exception(status.HTTP_400_BAD_REQUEST, api_msgs.PRODUCT_ITEM_ALREADY_EXISTS)
...
def get_product_item_or_raise_not_found(product_id: str, size_id: str, db: Session):
    ...
    raise_http_exception(status.HTTP_400_BAD_REQUEST, api_msgs.PRODUCT_ITEM_NOT_FOUND)
...
def get_size(size_id: str, db: Session):
    size = db.query(size_model.Size).filter(size_model.Size.id == size_id).first()
...
def size_exists(size_id: str, db: Session):
    ...
    raise_http_exception(status.HTTP_400_BAD_REQUEST, api_msgs.SIZE_NOT_FOUND)
