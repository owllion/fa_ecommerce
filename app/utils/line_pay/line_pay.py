    ...
    "packages": [
        {
            ...
            "name": "鬼滅之刃公仔",
            "products": [
                {"name": "竈門禰豆子", "quantity": 1, "price": 1000},
                {"name": "我妻善逸", "quantity": 1, "price": 1000},
            ],
        }
    ],
...
def do_confirm(transaction_id):
    ...
    response = requests.post(
        "https://sandbox-api-pay.line.me" + con_url, headers=headers, data=conf_data
    )
    print(response.text)
    response = json.loads(response.text)
